...
const Carousel = () => {
  ...
  return (
    ...
    >
      <div className="overflow-hidden relative h-72 mt-3 my-auto lg:-mx-40 ">
      ...
      <div className="absolute bottom-4 left-0 right-0 flex justify-center max-h-fit space-x-2 z-10">
        {images.map((_, index) => (
          <button
            key={index}
            className={`w-3 h-3 rounded-full ${index === currentIndex ? 'bg-gray-400' : 'bg-gray-200'}`}
